...
class CustomInfraDataset(DatasetTemplate):
    ...
    def include_data(self, mode):
        ...
        for info_path in self.dataset_cfg.INFO_PATH[mode]:
            ...
            with open(info_path, 'rb') as f:
                infos = pickle.load(f)
                custom_infra_infos.extend(infos)

        self.custom_infra_infos.extend(custom_infra_infos)
        self.logger.info('Total samples for Custom AV dataset: %d' % (len(custom_infra_infos)))
    ...
    def __len__(self):
        ...
        return len(self.custom_infra_infos)

    def __getitem__(self, index):
        if self._merge_all_iters_to_one_epoch:
            index = index % len(self.custom_infra_infos)

        info = copy.deepcopy(self.custom_infra_infos[index])
    ...
    def evaluation(self, det_annos, class_names, **kwargs):
        if 'annos' not in self.custom_infra_infos[0].keys():
        ...
        eval_gt_annos = [copy.deepcopy(info['annos']) for info in self.custom_infra_infos]
    ...
    def create_groundtruth_database(self, info_path=None, used_classes=None, split='train'):
        ...
        db_info_save_path = Path(self.root_path) / ('custom_infra_dbinfos_%s.pkl' % split)
...
def create_custom_infra_infos(dataset_cfg, class_names, data_path, save_path, workers=4):
    dataset = CustomInfraDataset(
    ...
    train_filename = save_path / ('custom_infra_infos_%s.pkl' % train_split)
    val_filename = save_path / ('custom_infra_infos_%s.pkl' % val_split)

    print('------------------------Start to generate data infos------------------------')

    dataset.set_split(train_split)
    custom_infra_infos_train = dataset.get_infos(
        class_names, num_workers=workers, has_label=True, num_features=num_features
    )
    with open(train_filename, 'wb') as f:
        pickle.dump(custom_infra_infos_train, f)
    print('custom_infra info train file is saved to %s' % train_filename)

    dataset.set_split(val_split)
    custom_infra_infos_val = dataset.get_infos(
        class_names, num_workers=workers, has_label=True, num_features=num_features
    )
    with open(val_filename, 'wb') as f:
        pickle.dump(custom_infra_infos_val, f)
    print('custom_infra info val file is saved to %s' % val_filename)
...
if __name__ == '__main__':
    ...
    if sys.argv.__len__() > 1 and sys.argv[1] == 'create_custom_infra_infos':
        import yaml
        ...
        create_custom_infra_infos(
            dataset_cfg=dataset_cfg,
            class_names=['Vehicle', 'Pedestrian', 'Cyclist'],
            data_path=ROOT_DIR / 'data' / 'custom_infra',
            save_path=ROOT_DIR / 'data' / 'custom_infra',
        )
    # import yaml
    # from pathlib import Path
    # from easydict import EasyDict

    # cfg_file = "/media/ssd_8tb/myGit/OpenPCDet/tools/cfgs/dataset_configs/custom_infra_dataset.yaml"
